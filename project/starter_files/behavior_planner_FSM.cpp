...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  s = vi /t + 0.5* a *t ^2
  */
  auto look_ahead_distance = 0.5 * (accel_mag) * pow((velocity_mag/accel_mag),2);  // <- Fix This
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      goal.velocity.x = 0.0;//_stop_threshold_speed * cos (ang);  // <- Fix This
      goal.velocity.y = 0.0;//_stop_threshold_speed * sin (ang) ;  // <- Fix This
